...
function secondReceptiveFieldMapping_2p(window)
...
% Start recording:
    ...
    % Receptive field 14*8 with Para_RFGrat.Contrast = Para_Monitor.MaximalContrast:
        % Each stimulation block is composed of 112 trials / position, 
        ...
        % Each trial is Para_RFGrat.ISI = 4000 ms.
            ...
            % Each stimulation(includes 4 cardinal orientation, full screen grating with grey patch) is Para_RFGrat.StimTime = 2000 ms; 
...
try
    ...
    moving_frames_angle = round(Para_RFGrat.StimTime/4000*FR); % for each cardinal orientation;
    
    %% Draw patches for each trail
    SpatFreqPix=1/(tand(1/Para_RFGrat.SpatFreqDeg/2)*Distance*2*Winwidth/Dispwidth);  
    f=SpatFreqPix*2*pi;
    Angles = [Para_RFGrat.Angle, Para_RFGrat.Angle+180,Para_RFGrat.Angle+90, Para_RFGrat.Angle+270];
    Angles = mod(Angles, 360);
    frames=round(FR/Para_RFGrat.TempFreq);
    phase=(1:frames)/frames*2*pi;  % grating
    movieFrameIndices=mod(0:(moving_frames_angle-1),frames) + 1;
    for t = 1:length(Angles) 
        a(t)=cosd(Angles(t)-180)*f;
        b(t)=sind(Angles(t)-180)*f;
    end
    
    % grating is full screen
    [x,y]=meshgrid(1:Winwidth,1:Winheight);
    rectwin_full=[0 0 Winwidth Winheight];
    
    % patch size in pixel, center of the patch
    diameter = round(tand(Para_RFGrat.patchSize/2)*Distance*2*Winwidth/Dispwidth); % grid size in pixels;

    % position of the grey patch
    for i=1:(Para_RFGrat.repetition*NumStim)
        x_pos=mod(sequence(i),Para_RFGrat.grid_width+1); % x = [0,13]; b = mod( a , m ) returns the remainder after division of a by m
        y_pos=fix(sequence(i)/(Para_RFGrat.grid_width+1)); % y = [0,7]; Y = fix( X ) rounds each element of X to the nearest integer toward zero.
        CenterX=round(x_pos*Para_RFGrat.grid_size_pixel+Para_RFGrat.grid_width_start);
        CenterY=round(y_pos*Para_RFGrat.grid_size_pixel+Para_RFGrat.grid_height_start);
    ...
    for t = 1:length(Angles)
        for k = 1:frames
            ...
            Para_RFGrat.tex(t,k)=Screen('MakeTexture', window, texdata_full);     
        end
    end
    ...
    for i=1:(Para_RFGrat.repetition*NumStim)
        ...
        for t = 1:length(Angles)
            for moving_frame_i = 1:moving_frames_angle
                Screen('DrawTexture', window, Para_RFGrat.tex(t,movieFrameIndices(moving_frame_i)), [], rectwin_full);
                Screen('FillOval', window, contrastpix(2), rectwin{i});
                Screen('Flip', window);
                [mX, mY, buttons] = GetMouse;
                if buttons(2)
                   Priority(0); 
